...
export class StockPageComponent  implements OnInit {
   ...
   async loadProduits() {
    try {
      ...
      this.produits = (produits || []).map(produit => ({
        ...produit,
        category: (produit as any).category || 'Non catégorisé' // Utilisez la catégorie existante ou une valeur par défaut
      }));
  ...
  getCategorie(produit: Produit): string {
    ...
    if ((produit as any).category) {
      return (produit as any).category;
    }
  ...
  filtrerProduits() {
    if (!this.searchTerm) {
      ...
      this.produitsFiltres = this.produits.filter(produit =>
        ...
        produit.category?.toLowerCase().includes(terme)
      );
    }
    this.grouperParCategorie();
  }
